...
void register_checker(void)
{
	...
	if (cpsr == 0b10010) {
		print(">>> Prozessor ist im IRQ-Modus => LR und SPSR verloren nach Interrupt\n"
	...
	print("Pruefe richtige Ruecksprung-Adresse (ca. 5 Sekunden)\n");
	_check_return_jump();

	print("Pruefe auf Register-Aenderungen (R0-R14, ca. 10 Sekunden)\n");
	_check_registers();
	...
	if (cpsr == 0b10000) {
		...
		print("Pruefe auf CPSR-Aenderungen (ca. 15 Sekunden)\n");
		_check_cpsr();
	}
	...
	if (cpsr == 0b10000 || cpsr == 0b11111) {
		...
		print("Pruefe auf SPSR-Aenderungen (ca. 15 Sekunden)\n");
		_check_spsr();
	}

	print("Fertig! Es _scheint_ so, als wuerde es funktionieren.\n");
}
